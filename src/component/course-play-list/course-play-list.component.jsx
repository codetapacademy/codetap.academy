...
import {
  mapLevel,
  getTotalSeconds,
  getPlayHistoryObject,
  getPercentage
} from './course-play-list.util';
...
import RangePointer from '../_dumb/range-pointer';
...
const CoursePlayList = ({ courseId }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const sectionListSnap = await sectionCollection.where('course.id', '==', courseId).get();
      const sectionList = [...sectionListSnap.docs]
        .map(doc => ({ ...doc.data(), id: doc.id }))
        .sort((a, b) => a.order - b.order);

      const lectureListSnap = await lectureCollection.where('course.id', '==', courseId).get();
      ...
      if (lectureList.length && lectureList[0]) {
        const initialLectureList = lectureList.filter(
          lecture => lecture.section.id === sectionList[0].id
        );
  ...
  const jumpToSecond = p => console.log(p) || videoRef.current.seekTo(p);

  const getFirstUnwatchedSecond = () => {
    const keyList = Object.keys(playedHistory.history || {});
    let minim = 0;
    if (keyList.length) {
      minim = Math.min(...keyList.filter(s => !playedHistory.history[s]).map(Number));
    }
    return minim ? minim / keyList.length : 0;
  };
  ...
  const onProgress = ({ playedSeconds }) => {
    ...
    //   currentSecond,
    //   playedSeconds,
    //   `${getPercentage(playedHistory.history)}%`,
    //   playedHistory
    // );
    if (currentSecond !== internalPlaySettings.current.ceilSeccond) {
      ...
      if (
        internalPlaySettings.current.currentPlayBeforeSave ===
        internalPlaySettings.current.saveSecondsInterval
      ) {
        updatePreviousVideo();
      ...
      const secondToUpdate =
        playedHistory.history && playedHistory.history.hasOwnProperty(currentSecond)
          ? currentSecond
          : 0;
  ...
  const updatePreviousVideo = () => {
    if (currentVideo && playedHistory.firebaseId) {
      ...
      playHistoryCollection.doc(firebaseId).set({ history }, { merge: true });
    }
  };
  ...
  return (
    <div>
      ...
      {course.version && course.version === 2 && (
        <StyledPlayerAndList>
          {currentVideo.vimeoVideoId && (
            <>
              <StyledReactPlayerWrapper>
                ...
                <RangePointer
                  onClick={() => jumpToSecond(getFirstUnwatchedSecond())}
                  percentage={getFirstUnwatchedSecond()}
                />
              </StyledReactPlayerWrapper>
            </>
          )}
